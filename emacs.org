...
*** Flycheck
#+begin_src emacs-lisp
      (defun ls/use-eslint-from-node-modules ()
      ...
      (use-package flycheck
            ...
            :hook
            (flycheck-mode . #'ls/use-eslint-from-node-modules))
...
#+begin_src emacs-lisp
    ...
    (use-package org
      ...
      (setq org-agenda-files
            (directory-files-recursively "~/Dropbox/main/" "\\.org$"))

      (setq org-refile-targets
            '(("~/Dropbox/main/org/archive.org" :maxlevel . 1)
              ("~/Dropbox/main/org/active.org" :maxlevel . 1)))
...
#+begin_src emacs-lisp
              (use-package org-roam
                :config
                (setq org-roam-directory "~/Dropbox/main/org/roam")
...
#+begin_src emacs-lisp
  ...
  const ${2:function_name}: React.FC<$2Props> = ({
      // props
  }) => {
  return (
...
** Relaxed
:PROPERTIES:
:header-args: :tangle ~/.emacs.d/themes/relaxed-theme.el
:END:
